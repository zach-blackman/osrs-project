...
def run_scan(params, progress=None, on_event=None):
    ...
    log(f"Liquid capital: {fmt_gp(params['bankroll'])} "
        f"· price floor {fmt_gp(params.get('min_price', 500_000))}")
    log("Fetching /mapping, /latest and /1h (three bulk calls)...")
    cands = prefilter(params["bankroll"], params["min_deploy_frac"],
                      params["max_spread_pct"], params["stale_hours"],
                      params.get("min_price", 500_000))
    log(f"{len(cands)} items passed liquidity + price + capacity filters", "ok")
    cands = cands[:params["shortlist"]]
    ...
    for i, c in enumerate(cands, 1):
        try:
            enriched = enrich(c, params["sleep"])
    ...
    if not rows:
        log("no items had enough history to score", "err")
        return []

    phase("Scoring, tax + capital economics, ranking")
    log(f"Computing after-tax margins and 24h fill limits for {len(rows)} items...")
    # Concrete economics per item, capital-aware. Buy at the insta-sell price
    # (low); sell into the current insta-buy price (high) for a safe fill.
    # margin is already net of the 2% GE tax (capped at 5m/item); gp_24h scales
    # that by how many units the buy limit + volume + your capital actually let
    # you flip in a day.
    ...
    for r in rows:
        ...
        r["units_24h"] = fillable_units_24h(r["limit"], low, r["vol_day"],
                                            bankroll)
        r["gp_24h"] = round(r["margin"] * r["units_24h"])

    score(rows)
    log("flip + swing sub-scores computed")
    merch_score(rows, news)
    log("composite merch score + catalysts applied", "ok")
    ...
    top = rows[:params["top"]]
    log(f"ranked — returning top {len(top)} picks", "ok")
    ...
    # Trim to a stable, JSON-serialisable shape for the GUI/CLI.
    keys = ("id", "name", "now", "margin", "roi", "gp_24h", "vol_day",
    ...
    return [{k: r.get(k) for k in keys} for r in top]
